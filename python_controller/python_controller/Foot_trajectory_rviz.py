...
class FootTrajectory_control(Node):
    def __init__(self):
        ...
        self.timer = self.create_timer(0.01, self.publish_trajectory) # Publish joint states at 10 Hz
        ...
        control_points = np.array([
            ...
            [0.05, -0.32],     # Mid lift
            [0.08, -0.32],     # Peak height
            [0.1, -0.3],     # Highest forward point
            [0.12, -0.3],      # Descending phase start
            [0.12, -0.3],      # Lowering further
            [0.12, -0.32],      # Almost touching ground
            [0.15, -0.32],    # Landing position
            [0.15, -0.32],    # Slight after-movement
            [0.18, -0.35],     # Return to stable position
            [0.18, -0.35],     # Final adjustment (end position)
        ])

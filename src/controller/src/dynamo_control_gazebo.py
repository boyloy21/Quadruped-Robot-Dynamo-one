...
# Robot geometry
body = [0.433, 0.295]
legs = [0., 0.087, 0.25, 0.25] 

Dynamo_one = Robot(body, legs, USE_IMU)
...
class Controller(Node):
    ...
    def timer_callback(self):
        leg_positions = Dynamo_one.run()
        Dynamo_one.change_controller()

        dx = Dynamo_one.state.body_local_position[0]
        dy = Dynamo_one.state.body_local_position[1]
        dz = Dynamo_one.state.body_local_position[2]
    
        roll = Dynamo_one.state.body_local_orientation[0]
        pitch = Dynamo_one.state.body_local_orientation[1]
        yaw = Dynamo_one.state.body_local_orientation[2]
...
class Joy_Subscriber(Node):
    ...
    def __init__(self):
        ...
        self.subscription = self.create_subscription(
            Joy,
            'dynamo_one/ramped_joy',
            Dynamo_one.joystick_command,
            10)
        self.subscription

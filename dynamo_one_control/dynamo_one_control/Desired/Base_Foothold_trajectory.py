...
from dynamo_one_control.utills.mpc_utill import rot_to_rpy
...
class Base_Foot_Trajectory():
    def __init__(self, L, W, dt=0.02, k=0.03, hf=0.05):
        ...
        self.Phip = np.array([[L/2, self.L1 + W/2, 0.0],
                             [L/2,-self.L1 - W/2, 0.0],
                             [-L/2, -self.L1 - W/2, 0.0],
                             [-L/2, self.L1 + W/2, 0.0]])
    ...
    def Body_motion_ref(self, Vcmd, Omegacmd, robot_height=0.3):
        ...
        theta = rot_to_rpy(self.Rb_cur)
        

        return self.Pb, Vb, theta, Omegab
    
    def FootTarget(self, Pb, Vcur, Vcmd, Tst, legID = 0):
        """Calculate next foot placement target"""
        # Current foot position in world frame
        current_foot_pos = Pb + self.Rb_cur @ self.Phip[legID]
        current_foot_pos[2] = 0.0
        ...
        correction_term = self.k * (Vcur - Vcmd)

        return current_foot_pos + target_offset + correction_term
    ...
    def SwingTrajectory(self, Ps, Pe, phi, Tsw):
        """

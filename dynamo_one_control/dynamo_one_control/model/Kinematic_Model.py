import numpy as np
from math import *
from dynamo_one_control.utills.kinematic_utill import point_to_rad, RotMatrix3D
...
class KinematicQuadruped():
    ...
    def InverseKinematic(self, Xf, Yf, Zf, right = False):
        ...
        # cos_val = (self.L2**2 + len_B**2 - self.L3**2) / (2 * self.L2 * len_B)
        # cos_val = np.clip(cos_val, -1.0, 1.0)  # Prevent invalid acos inputs
        # b_2 = np.arccos(cos_val)
    ...
    def jacobian(self, angles, legID=0):
        ...
        if is_right:
            ...
            sign = -1
        theta2_adj = -theta2 - 55*pi/180 + 1.5 * pi
        theta3_adj = -theta3 
        ...
        if np.any(np.isnan(J)):
            print(f"Invalid Jacobian at angles: {angles}")
    ...
    #     J = self.jacobian_numrical(angles, leg_id)
        
        
    #     # 2. Use damped least squares for stability
    #     # damping = 1e-6
    #     # J_T = J.T
    #     # inv_J = J_T @ np.linalg.inv(J @ J_T + damping * np.eye(3))
        
    #     joint_velocities = np.linalg.inv(J) @ np.array(foot_velocity).reshape(3, 1)
    #     return joint_velocities.flatten()
    
    # def get_joint_velocity(self, foot_velocity, angles, leg_id=0):
    #     """Improved joint velocity calculation using robust Jacobian"""
    #     try:
    ...
    #         # Calculate joint velocities with clipping
    #         q_dot = inv_J @ np.asarray(foot_velocity).reshape(3, 1)
    #         return np.clip(q_dot.flatten(), -3.0, 3.0)  # rad/s limits
            
    #     except Exception as e:
    #         self.get_logger().error(f"Velocity calculation failed: {str(e)}")
    #         return np.zeros(3)
        
    def get_joint_velocity(self, foot_velocity, angles, leg_id=0):
        """Robust joint velocity calculation with:
        - Analytical Jacobian
        ...
        try:
            ...
            # 2. Damped least squares for singularity robustness
            J_T = J.T
            inv_term = J @ J_T + DAMPING * np.eye(3)
            
            try:
                inv_J = J_T @ np.linalg.inv(inv_term)
            except np.linalg.LinAlgError:
                print(f"Singularity detected in leg {leg_id}")
                return np.zeros(3)
            
            # 3. Calculate joint velocities
            foot_vel = np.asarray(foot_velocity).reshape(3, 1)
            q_dot = inv_J @ foot_vel
            
            # 4. Apply velocity limits
            q_dot_norm = np.linalg.norm(q_dot)
            if q_dot_norm > MAX_JOINT_VEL:
                q_dot = q_dot * (MAX_JOINT_VEL / q_dot_norm)
                
            return q_dot.flatten()
            
        except Exception as e:
            # print(f"Velocity calculation failed in leg {leg_id}: {str(e)}")
            return np.zeros(3)
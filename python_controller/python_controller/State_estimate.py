...
from geometry_msgs.msg import Wrench, Vector3, Quaternion, Pose, Point
from nav_msgs.msg import Odometry
from sensor_msgs.msg import Imu, JointState
from python_controller.Controller.Kinematic_Model import KinematicQuadruped
from python_controller.Filter.moving_window_filter import MovingWindowFilter
...
class Sensor_Feedback(Node):
    def __init__(self):
        ...
        self.odom_pub = self.create_publisher(Odometry, 'dynamo_one/odom', 10)
        ...
        # CPG force publisher
        self.force_pub = self.create_publisher(LegForces, '/cpg/leg_forces', 10)
    ...
    def publish_odometry(self):
        ...
        self.publish_leg_forces()

    def publish_leg_forces(self):
        """Publish all leg forces in a single message"""
        msg = LegForces()
        
        # FL
        msg.fl_x = float(self.FL_force[0])
        msg.fl_y = float(self.FL_force[1])
        msg.fl_z = float(self.FL_force[2])
        
        # FR
        msg.fr_x = float(self.FR_force[0])
        msg.fr_y = float(self.FR_force[1])
        msg.fr_z = float(self.FR_force[2])
        
        # RL
        msg.rl_x = float(self.RL_force[0])
        msg.rl_y = float(self.RL_force[1])
        msg.rl_z = float(self.RL_force[2])
        
        # RR
        msg.rr_x = float(self.RR_force[0])
        msg.rr_y = float(self.RR_force[1])
        msg.rr_z = float(self.RR_force[2])
        
        self.force_pub.publish(msg)
        self.get_logger().debug("Published leg forces to CPG", throttle_duration_sec=1.0)
